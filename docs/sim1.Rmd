...
The simulations generate a population the size of the student body, and place them into hierarchies that represent location and living circle. Individuals are initially sampled randomly to be positive based on some baseline prevalence. The number of individuals that each individual infects is possonly distributed with some value of $\lambda$ that represents the average number infected per individual. Then they transmit to some number of other students based on the following probabilities:
...
Optionally, we can follow up positive pools by individually testing everyone within that pool.
...
- Costs of sample kits and tests

...
$$
\arg{{\min\below{\alpha,\beta}{\left(Q\left(D=1,R_0,C_t=35,\alpha,\beta,T_{se}=0.02\right)-Q\left(D=10,R_0,C_t=35,\alpha,\beta,T_{se}=0.2\right)\right)}}^2}
$$
...
ress <- res %>%
	...
	summarise(
		...
		efficiency.ind_lfd = mean(sensitivity.ind_tests / cost.ind_tests)
	)
...
temp <- group_by(ress, prevalence, spread, containment) %>%
	summarise(
		...
		ppv.ind_tests=mean(ppv.ind_tests),
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(cost.ind_tests, cost.pool_tests, cost.poolfollowup_tests, cost.ind_lfd, cost.ind_tests, group, containment, pool_size, prevalence, spread) %>%
tidyr::gather(key="key", value="cost", cost.pool_tests, cost.poolfollowup_tests, cost.ind_tests, cost.ind_lfd) %>%
subset(., grepl("pool", key) | grepl("ind", key) & pool_size == 1) %>%
mutate(key=rename_labels(key), spread=paste("R =", spread), prevalence=paste("prv =", prevalence)) %>%
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(reagentuse.ind_tests, reagentuse.ind_lfd, reagentuse.pool_tests, reagentuse.poolfollowup_tests, group, containment, pool_size, prevalence, spread) %>%
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(sensitivity.ind_tests, sensitivity.pool_tests, sensitivity.poolfollowup_tests, sensitivity.ind_lfd, group, containment, pool_size, prevalence, spread) %>%
tidyr::gather(key="key", value="sensitivity", sensitivity.pool_tests, sensitivity.poolfollowup_tests, sensitivity.ind_tests, sensitivity.ind_lfd) %>%
subset(., grepl("pool", key) | grepl("ind", key) & pool_size == 1) %>%
mutate(key=rename_labels(key), spread=paste("R =", spread), prevalence=paste("prv =", prevalence)) %>%
...
scale_fill_brewer() +
theme_bw() +
labs(x="Pool size", y="Sensitivity of pooled PCR with followup", fill="Random pooling")
```
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(specificity.ind_tests, specificity.ind_lfd, specificity.pool_tests, specificity.poolfollowup_tests, group, containment, pool_size, prevalence, spread) %>%
tidyr::gather(key="key", value="specificity", specificity.pool_tests, specificity.poolfollowup_tests, specificity.ind_tests, specificity.ind_lfd) %>%
subset(., grepl("pool", key) | grepl("ind", key) & pool_size == 1) %>%
mutate(key=rename_labels(key), spread=paste("R =", spread), prevalence=paste("prv =", prevalence)) %>%
ggplot(., aes(x=as.factor(pool_size), y=1-specificity, group=group)) +
geom_bar(position="dodge2", stat="identity", aes(fill=key)) +
facet_grid(prevalence ~ spread) +
scale_fill_brewer(type="qual", palette=2) +
theme_bw() +
scale_y_log10() +
labs(x="Pool size", y="False positive rate (log scale, lower is better)")
```
...
facet_grid(prevalence ~ spread) +
scale_colour_brewer() +
labs(x="Pool size", y="Specificity", colour="R", shape="Random pooling", linetype="Random pooling")
```
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(ppv.ind_tests, ppv.ind_lfd, ppv.pool_tests, ppv.poolfollowup_tests, group, containment, pool_size, prevalence, spread) %>%
tidyr::gather(key="key", value="ppv", ppv.pool_tests, ppv.poolfollowup_tests, ppv.ind_tests, ppv.ind_lfd) %>%
subset(., grepl("pool", key) | grepl("ind", key) & pool_size == 1) %>%
mutate(key=rename_labels(key), spread=paste("R =", spread), prevalence=paste("prv =", prevalence)) %>%
ggplot(., aes(x=as.factor(pool_size), y=ppv, group=group)) +
geom_bar(position="dodge2", stat="identity", aes(fill=key)) +
facet_grid(prevalence ~ spread) +
scale_fill_brewer(type="qual", palette=2) +
theme_bw() +
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(prevalence.ind_tests, prevalence.ind_lfd, prevalence.pool_tests, prevalence.poolfollowup_tests, group, containment, pool_size, prevalence, spread, true_prevalence) %>%
tidyr::gather(key="key", value="est_prevalence", prevalence.pool_tests, prevalence.poolfollowup_tests, prevalence.ind_tests, prevalence.ind_lfd) %>%
subset(., grepl("pool", key) | grepl("ind", key) & pool_size == 1) %>%
...
facet_wrap(~ key + prevalence, ncol=3, scale="free") +
geom_abline(slope=1) +
scale_colour_brewer(type="qual") +
theme_bw() +
labs(x="True prevalence", y="Estimated prevalence", colour="Pool size")
```
...
```{r}
ress2 %>% subset(., !random_pooling & containment == "high") %>%
dplyr::select(efficiency.ind_tests, efficiency.ind_lfd, efficiency.pool_tests, efficiency.poolfollowup_tests, group, containment, pool_size, prevalence, spread) %>%
tidyr::gather(key="key", value="efficiency", efficiency.pool_tests, efficiency.poolfollowup_tests, efficiency.ind_tests, efficiency.ind_lfd) %>%
subset(., grepl("pool", key) | grepl("ind", key) & pool_size == 1) %>%
mutate(key=rename_labels(key), spread=paste("R =", spread), prevalence=paste("prv =", prevalence)) %>%
...
scale_fill_brewer(type="qual", palette=2) +
theme_bw() +
labs(x="Pool size", y="Sensitivity / cost")
```
